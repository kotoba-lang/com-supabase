(ns supabase.main
  "Kotodama WASM entrypoint for the Supabase clean-room actor (L4) — Clojure port.
  ...
  facts (`supabase.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "supabase")
(def tier "L4")
...
(def entity-specs
  [{:entity "Project"      :plural "projects"      :id-prefix "supabase_pro"
    :fields [:ref :organizationSlug :name :region :status]  :required [:ref]
    :coerce {}                                  :refs {}}
   {:entity "Organization" :plural "organizations" :id-prefix "supabase_org"
    :fields [:slug :name]                       :required [:slug]
    :coerce {}                                  :refs {}}
   {:entity "EdgeFunction" :plural "edgefunctions" :id-prefix "supabase_edg"
    :fields [:slug :name :status :version :verifyJwt :entrypointPath]
    :required [:slug]
    :coerce {:version :int :verifyJwt :bool}   :refs {}}
   {:entity "ApiKey"       :plural "apikeys"       :id-prefix "supabase_api"
    :fields [:type :name :description :prefix :insertedAt]
    :required [:type]
    :coerce {}                                  :refs {}}
   {:entity "Branch"       :plural "branches"      :id-prefix "supabase_bra"
    :fields [:name :projectRef :parentProjectRef :isDefault :persistent :status :previewProjectStatus]
    :required [:name]
    :coerce {:isDefault :bool :persistent :bool} :refs {}}
   {:entity "Backup"       :plural "backups"       :id-prefix "supabase_bac"
    :fields [:status :isPhysicalBackup :insertedAt]
    :required [:status]
    :coerce {:isPhysicalBackup :bool}  :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"supabase.<Entity>/<field>\" v ...}. The datomic
...
(defn handle-create [store entity data]
  (let [{:keys [fields required coerce id-prefix]} (spec-for entity)
        allowed (set (into fields [:id :createdAt :updatedAt]))]
    (or (some-> (reject-unknown data (vec allowed)) (vector 400))
...
(defn healthz [] [{:status "ok" :actor "supabase-compat" :tier tier :entities entities} 200])
...
(defn start! [] :supabase-compat/ready)
